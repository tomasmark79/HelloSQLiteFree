 ...
 * @file    Column.h
 * @ingroup SQLiteCpp
 * @brief   Encapsulation of a Column in a row of the result pointed by the
...
namespace SQLite
{
   ...
   *
   * Thread-safety: a Column object shall not be shared by multiple threads,
  ...
  public:
     ...
     *
     * @param[in] aStmtPtr  Shared pointer to the prepared SQLite Statement
     * Object.
    ...
    /// Return the 32bits unsigned integer value of the column (note that
    /// SQLite3 does not support unsigned 64bits).
    uint32_t getUInt () const noexcept;
    /// Return the 64bits integer value of the column (note that SQLite3 does
    /// not support unsigned 64bits).
    int64_t getInt64 () const noexcept;
    /// Return the double (64bits float) value of the column
    double getDouble () const noexcept;
    /**
     * @brief Return a pointer to the text value (NULL terminated string) of
     * the column.
     *
     * @warning The value pointed at is only valid while the statement is valid
    ...
    /**
     * @brief Return the type of the value of the column using
     * sqlite3_column_type()
     *
     * Return either SQLite::INTEGER, SQLite::FLOAT, SQLite::TEXT,
     * SQLite::BLOB, or SQLite::Null. This type may change from one row to the
     * next, since SQLite stores data types dynamically for each value and not
     * per column. Use Statement::getColumnDeclaredType() to retrieve the
     * declared column type from a SELECT statement.
     *
     * @warning After a type conversion (by a call to a getXxx on a Column of a
     * Yyy type), the value returned by sqlite3_column_type() is undefined.
     */
     ...
     *
     * Return either :
     * - size in bytes (not in characters) of the string returned by getText()
     * without the '\0' terminator
     * - size in bytes of the string representation of the numerical value
     * (integer or double)
   ...
   *
   * Insert the text value of the Column object, using getText(), into the
   * provided stream.
   *
