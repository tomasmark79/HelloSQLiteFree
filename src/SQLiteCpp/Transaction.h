 ...
 * @file    Transaction.h
 * @ingroup SQLiteCpp
 * @brief   A Transaction is way to group multiple SQL statements into an
 * atomic secured operation.
 *
...
namespace SQLite
{
   ...
   *
   * A Transaction is a way to group multiple SQL statements into an atomic
   * secured operation; either it succeeds, with all the changes committed to
   * the database file, or if it fails, all the changes are rolled back to the
   * initial state.
   *
   ...
   *
   * Thread-safety: a Transaction object shall not be shared by multiple
   * threads, because : 1) in the SQLite "Thread Safe" mode, "SQLite can be
   * safely used by multiple threads provided that no single database
   * connection is used simultaneously in two or more threads." 2) the SQLite
   * "Serialized" mode is not supported by SQLiteC++, because of the way it
