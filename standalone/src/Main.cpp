...
namespace Config {
  constexpr char standaloneName[] = "HelloHelloSQLite";
...
std::unique_ptr<dotname::HelloSQLite> uniqueLib;
...
int processArguments (int argc, const char* argv[]) {
  try {
    ...
    if (!result.count ("omit")) {
      // uniqueLib = std::make_unique<dotname::HelloSQLite> ();
      uniqueLib = std::make_unique<dotname::HelloSQLite> (Config::assetsPath);
    } else {
