 ...
 * @file    ExecuteMany.h
 * @ingroup SQLiteCpp
 * @brief   Convenience function to execute a Statement with multiple Parameter
 * sets
 *
...
namespace SQLite {
  ...
  /**
   * \brief Convenience function to execute a Statement with multiple Parameter
   * sets once for each parameter set given.
   *
   *
  ...
  void execute_many (Database& aDatabase, const char* apQuery, Arg&& aArg, Types&&... aParams) {
    ...
    (void)std::initializer_list<int>{ ((void)reset_bind_exec (query, std::forward<Types> (aParams)),
                                       0)... };
  }
   ...
   */
  template <typename TupleT> void reset_bind_exec (Statement& apQuery, TupleT&& aTuple) {
    apQuery.reset ();
   ...
   * @param aTuple    Tuple to bind
   */
  template <typename TupleT> void bind_exec (Statement& apQuery, TupleT&& aTuple) {
