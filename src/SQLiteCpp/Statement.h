 ...
 * @file    Statement.h
 * @ingroup SQLiteCpp
 * @brief   A prepared SQLite Statement is a compiled SQL query ready to be
 * executed, pointing to a row of result.
 *
...
namespace SQLite
{
   ...
   * is compiled in the constructor and finalized in the destructor, so that
   * there is no need to worry about memory management or the validity of the
   * underlying SQLite Statement.
   *
   * Thread-safety: a Statement object shall not be shared by multiple threads,
   * because : 1) in the SQLite "Thread Safe" mode, "SQLite can be safely used
   * by multiple threads provided that no single database connection is used
   * simultaneously in two or more threads." 2) the SQLite "Serialized" mode is
   * not supported by SQLiteC++, because of the way it shares the underling
  ...
  public:
    /**
     * @brief Compile and register the SQL query for the provided SQLite
     * Database Connection
     *
     * @param[in] aDatabase the SQLite Database Connection
     * @param[in] apQuery   an UTF-8 encoded query string
     *
     * Exception is thrown in case of error, then the Statement object is NOT
     * constructed.
     */
    ...
    /// Finalize and unregister the SQL query from the SQLite Database
    /// Connection. The finalization will be done by the destructor of the last
    /// shared pointer
    ~Statement () = default;

    /// Reset the statement to make it ready for a new execution by calling
    /// sqlite3_reset. Throws an exception on error. Call this function before
    /// any news calls to bind() if the statement was already executed before.
    /// Calling reset() does not clear the bindings (see clearBindings()).
    void reset ();

    /// Reset the statement. Returns the sqlite result code instead of throwing
    ...
    // which tell the sqlite library to make its own copy of the data before
    // the bind() call returns. This choice is done to prevent any common
    // misuses, like passing a pointer to a dynamic allocated and temporary
    // variable (a std::string for instance). This is under-optimized for
    // static data (a static text define in code) as well as for dynamic
    // allocated buffer which could be transfer to sqlite instead of being
    // copied.
    ...
    /**
     * @brief Bind an int value to a parameter "?", "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    void bind (const int aIndex, const int32_t aValue);
    /**
     * @brief Bind a 32bits unsigned int value to a parameter "?", "?NNN",
     * ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    void bind (const int aIndex, const uint32_t aValue);
    /**
     * @brief Bind a 64bits int value to a parameter "?", "?NNN", ":VVV",
     * "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    void bind (const int aIndex, const int64_t aValue);
    /**
     * @brief Bind a double (64bits float) value to a parameter "?", "?NNN",
     * ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    void bind (const int aIndex, const double aValue);
    /**
     * @brief Bind a string value to a parameter "?", "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @note Uses the SQLITE_TRANSIENT flag, making a copy of the data, for
     * SQLite internal use
     */
    void bind (const int aIndex, const std::string &aValue);
    /**
     * @brief Bind a text value to a parameter "?", "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @note Uses the SQLITE_TRANSIENT flag, making a copy of the data, for
     * SQLite internal use
    ...
    /**
     * @brief Bind a binary blob value to a parameter "?", "?NNN", ":VVV",
     * "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @note Uses the SQLITE_TRANSIENT flag, making a copy of the data, for
     * SQLite internal use
     ...
     *
     * @warning Uses the SQLITE_STATIC flag, avoiding a copy of the data. The
     * string must remains unchanged while executing the statement.
     */
     ...
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * Main usage is with null-terminated literal text (aka in code static
     * strings)
     *
     * @warning Uses the SQLITE_STATIC flag, avoiding a copy of the data. The
     * string must remains unchanged while executing the statement.
    ...
    /**
     * @brief Deleted, because the value's lifetime could not be guaranteed.
     * Use bind().
     */
    void bindNoCopy (const int aIndex, std::string &&aValue) = delete;

    /**
     * @brief Bind a NULL value to a parameter "?", "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
    ...
    /**
     * @brief Bind an int value to a named parameter "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    ...
    /**
     * @brief Bind a 32bits unsigned int value to a named parameter "?NNN",
     * ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    ...
    /**
     * @brief Bind a 64bits int value to a named parameter "?NNN", ":VVV",
     * "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    ...
    /**
     * @brief Bind a double (64bits float) value to a named parameter "?NNN",
     * ":VVV", "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     */
    ...
    /**
     * @brief Bind a string value to a named parameter "?NNN", ":VVV", "@VVV"
     * or "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @note Uses the SQLITE_TRANSIENT flag, making a copy of the data, for
     * SQLite internal use
     */
    ...
    /**
     * @brief Bind a text value to a named parameter "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @note Uses the SQLITE_TRANSIENT flag, making a copy of the data, for
     * SQLite internal use
    ...
    /**
     * @brief Bind a binary blob value to a named parameter "?NNN", ":VVV",
     * "@VVV" or "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @note Uses the SQLITE_TRANSIENT flag, making a copy of the data, for
     * SQLite internal use
    ...
    /**
     * @brief Bind a NULL value to a named parameter "?NNN", ":VVV", "@VVV" or
     * "$VVV" in the SQL prepared statement (aIndex >= 1)
     *
     * @see clearBindings() to set all bound parameters to NULL.
     */
     ...
     * string must remains unchanged while executing the statement.
     */
    void bindNoCopy (const std::string &aName, const void *apValue,
                     const int aSize)
    {
     ...
     * accessed through the getColumn() method
     *
     * @see exec() execute a one-step prepared statement with no expected
     * result
     * @see tryExecuteStep() try to execute a step of the prepared query to
     * fetch one row of results, returning the sqlite result code.
     * @see Database::exec() is a shortcut to execute one or multiple
     * statements without results
     *
     * @return - true  (SQLITE_ROW)  if there is another row ready : you can
     * call getColumn(N) to get it then you have to call executeStep() again to
     * fetch more rows until the query is finished
     *         - false (SQLITE_DONE) if the query has finished executing :
     * there is no (more) row of result (case of a query with no result, or
     * after N rows fetched successfully)
     *
    ...
    /**
     * @brief Try to execute a step of the prepared query to fetch one row of
     * results, returning the sqlite result code.
     *
     *
     *
     * @see exec() execute a one-step prepared statement with no expected
     * result
     * @see executeStep() execute a step of the prepared query to fetch one row
     * of results
     * @see Database::exec() is a shortcut to execute one or multiple
     * statements without results
     *
    ...
    /**
     * @brief Execute a one-step query with no expected result, and return the
     * number of changes.
     *
     *  This method is useful for any kind of statements other than the Data
     * Query Language (DQL) "SELECT" :
     *  - Data Definition Language (DDL) statements "CREATE", "ALTER" and
     * "DROP"
     *  - Data Manipulation Language (DML) statements "INSERT", "UPDATE" and
     * "DELETE"
     *  - Data Control Language (DCL) statements "GRANT", "REVOKE", "COMMIT"
     * and "ROLLBACK"
     *
     * It is similar to Database::exec(), but using a precompiled statement, it
     * adds :
     ...
     * statements without results
     *
     * @return number of row modified by this SQL statement (INSERT, UPDATE or
     * DELETE)
     *
     * @throw SQLite::Exception in case of error, or if row of results are
     * returned while they are not expected!
     */
    int exec ();
     ...
     *
     *  Can be used to access the data of the current row of result when
     * applicable, while the executeStep() method returns true.
     *
     ...
     * - after a reset() call
     *
     *  Throw an exception if the specified index is out of the [0,
     * getColumnCount()) range.
     *
     * @param[in] aIndex    Index of the column, starting at 0
     *
     * @note    This method is not const, reflecting the fact that the returned
     * Column object will share the ownership of the underlying sqlite3_stmt.
     *
     * @warning The resulting Column object must not be memorized "as-is".
     *          Is is only a wrapper around the current result row, so it is
     * only valid while the row from the Statement remains valid, that is only
     * until next executeStep() call. Thus, you should instead extract
     * immediately its data (getInt(), getText()...) and use or copy this data
     * for any later usage.
     */
     ...
     * (less efficient than using an index)
     *
     *  Can be used to access the data of the current row of result when
     * applicable, while the executeStep() method returns true.
     *
     ...
     * - after a reset() call
     *
     *  Throw an exception if the specified name is not an on of the aliased
     * name of the columns in the result.
     *
     * @param[in] apName   Aliased name of the column, that is, the named
     * specified in the query (not the original name)
     *
     ...
     * for any later usage.
     *
     *  Throw an exception if the specified name is not one of the aliased name
  ...
  || (defined(_MSC_VER) && _MSC_VER >= 1900) // c++14: Visual Studio 2015
                                             /**
                                              * @brief Return an instance of T constructed from copies of the first N
                                              * columns
                                              *
                                              *  Can be used to access the data of the current row of result when
                                              * applicable,                                          while the executeStep() method returns true.
                                              ...
                                              * - after a reset() call
                                              *
                                              *  Throw an exception if the specified column count is out of the [0,
                                              * getColumnCount()) range.
                                              *
                                              ...
                                              */
    template <typename T, int N> T getColumns ();

  private:
    /**
     * @brief Helper function used by getColumns<typename T, int N> to expand
     * an integer_sequence used to generate the required Column objects
     */
  ...
  public:
    ...
    /**
     * @brief Return a pointer to the named assigned to the specified result
     * column (potentially aliased)
     *
     * @param[in] aIndex    Index of the column in the range [0,
     * getColumnCount()).
     *
    ...
    /**
     * @brief Return a pointer to the table column name that is the origin of
     * the specified result column
     *
     *  Require definition of the SQLITE_ENABLE_COLUMN_METADATA preprocessor
     * macro :
     * - when building the SQLite library itself (which is the case for the
    ...
    /**
     * @brief Return the index of the specified (potentially aliased) column
     * name
     *
     * @param[in] apName    Aliased name of the column, that is, the named
     * specified in the query (not the original name)
    ...
    /**
     * @brief Return the declared type of the specified result column for a
     * SELECT statement.
     *
     *  This is the type given at creation of the column and not the actual
     * data type. SQLite stores data types dynamically for each value and not
     * per column.
     *
     * @param[in] aIndex    Index of the column in the range [0,
     * getColumnCount()).
     ...
     *  - the column at aIndex is not a table column but an expression or
     * subquery
     */
    ...
    // Return a UTF-8 string containing the SQL text of prepared statement with
    ...
    bool hasRow () const { return mbHasRow; }
  ...
  private:
    /**
     * @brief Check if a return code equals SQLITE_OK, else throw a
     * SQLite::Exception with the SQLite error message
     *
     * @param[in] aRet SQLite return code to test against the SQLITE_OK
     * expected value
     */
    ...
    /**
     * @brief Check if there is a row of result returned by executeStep(), else
    ...
    void checkRow () const
    {
      if (false == mbHasRow)
      {
        throw SQLite::Exception ("No row to get a column from. executeStep() "
                                 "was not called, or returned false.");
      }
    }

    /**
     * @brief Check if there is a Column index is in the range of columns in
     * the result.
     */
    ...
    TStatementPtr mpPreparedStatement; //!< Shared Pointer to the prepared
                                       //!< SQLite Statement Object
    int mColumnCount
      = 0; //!< Number of columns in the result of the prepared statement
    bool mbHasRow
      = false; //!< true when a row has been fetched with executeStep()
    bool mbDone
