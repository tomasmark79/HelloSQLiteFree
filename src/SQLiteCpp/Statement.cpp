...
namespace SQLite {
  ...
  void Statement::bind (const int aIndex, const char* apValue) {
    const int ret
        = sqlite3_bind_text (getPreparedStatement (), aIndex, apValue, -1, SQLITE_TRANSIENT);
    check (ret);
  }
  ...
  void Statement::bind (const int aIndex, const void* apValue, const int aSize) {
    const int ret
        = sqlite3_bind_blob (getPreparedStatement (), aIndex, apValue, aSize, SQLITE_TRANSIENT);
    check (ret);
  }
  ...
  void Statement::bindNoCopy (const int aIndex, const void* apValue, const int aSize) {
    const int ret
        = sqlite3_bind_blob (getPreparedStatement (), aIndex, apValue, aSize, SQLITE_STATIC);
    check (ret);
  }
  ...
  bool Statement::executeStep () {
    ...
    if ((SQLITE_ROW != ret)
        && (SQLITE_DONE != ret)) // on row or no (more) row ready, else it's a problem
    {
  ...
  Statement::TStatementPtr Statement::prepareStatement () {
    ...
    const int ret = sqlite3_prepare_v2 (mpSQLite, mQuery.c_str (),
                                        static_cast<int> (mQuery.size ()), &statement, nullptr);
    if (SQLITE_OK != ret) {
      throw SQLite::Exception (mpSQLite, ret);
    }
