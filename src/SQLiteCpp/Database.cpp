...
// check if sqlite version is >= 3.31.0 and SQLITE_OPEN_NOFOLLOW is defined
  ...
  // Shortcut to execute a one step query and fetch the first column of the
  // result. WARNING: Be very careful with this dangerous method: you have to
  ...
  // Attach a custom function to your sqlite database. Assumes UTF8 text
  ...
  void Database::loadExtension (const char* apExtensionName, const char* apEntryPointName) {
    ...
    // be used to enable only this interface. The use of the
    // sqlite3_enable_load_extension() interface should be avoided to keep the
    // SQL load_extension() disabled and prevent SQL injections from giving
  ...
  Header Database::getHeaderInfo (const std::string& aFilename) {
    ...
    // If the "magic string" can't be found then header is invalid, corrupt or
    // unreadable
