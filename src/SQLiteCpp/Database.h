...
// c++17: macOS unless targetting compatibility with macOS < 10.15
    ...
    // macOS clang won't let us touch std::filesystem if we're targetting
    // earlier than 10.15
    #elif defined(__IPHONE_OS_VERSION_MIN_REQUIRED) && defined(__IPHONE_13_0) \
      && __IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_13_0
    // build for iOS clang won't let us touch std::filesystem if we're
    // targetting earlier than iOS 13
    #else
      #define SQLITECPP_HAVE_STD_FILESYSTEM
    #endif
...
#ifndef SQLITE_USE_LEGACY_STRUCT // Since SQLITE 3.19 (used by default since
                                 // SQLiteCpp 2.1.0)
typedef struct sqlite3_value sqlite3_value;
#else // Before SQLite 3.19 (legacy struct forward declaration can be activated
      // with CMake SQLITECPP_LEGACY_STRUCT var)
struct Mem;
...
namespace SQLite
{
  ...
  // Those public constants enable most usages of SQLiteCpp without including
  // <sqlite3.h> in the client application.
  ...
  /// With OPEN_READWRITE: The database is opened for reading and writing, and
  /// is created if it does not already exist.
  SQLITECPP_API extern const int OPEN_CREATE; // SQLITE_OPEN_CREATE
  /// Enable URI filename interpretation, parsed according to RFC 3986 (ex.
  ...
  /// Database filename is not allowed to be a symbolic link (Note: only since
  ...
  SQLITECPP_API extern const char *const
    VERSION; ///< SQLITE_VERSION string from sqlite3.h used at compile time
  SQLITECPP_API extern const int
    VERSION_NUMBER; ///< SQLITE_VERSION_NUMBER from sqlite3.h used at compile
                    ///< time
  ...
  // Public structure for representing all fields contained within the SQLite
  // header. Official documentation for fields:
   ...
   *
   * A Database object manage a list of all SQLite Statements associated with
   * the underlying SQLite 3 database connection.
   *
   * Resource Acquisition Is Initialization (RAII) means that the Database
   * Connection is opened in the constructor and closed in the destructor, so
   * that there is no need to worry about memory management or the validity of
   * the underlying SQLite Connection.
   *
   * Thread-safety: a Database object shall not be shared by multiple threads,
  ...
  {
    friend class Statement; // Give Statement constructor access to the
                            // mSQLitePtr Connection Handle

  public:
    /**
     * @brief Open the provided database UTF-8 filename.
     *
     * Uses sqlite3_open_v2() with readonly default flag, which is the opposite
     * behavior of the old sqlite3_open() function (READWRITE+CREATE). This
     * makes sense if you want to use it on a readonly filesystem or to prevent
     * creation of a void file when a required file is missing.
     *
     * Exception is thrown in case of error, then the Database object is NOT
     * constructed.
     *
     * @param[in] apFilename        UTF-8 path/uri to the database file
     * ("filename" sqlite3 parameter)
     * @param[in] aFlags
     * SQLite::OPEN_READONLY/SQLite::OPEN_READWRITE/SQLite::OPEN_CREATE...
     * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before
     * returning SQLITE_BUSY (see setBusyTimeout())
     * @param[in] apVfs             UTF-8 name of custom VFS to use, or nullptr
     * for sqlite3 default
     *
     * @throw SQLite::Exception in case of error
     */
    Database (const char *apFilename, const int aFlags = SQLite::OPEN_READONLY,
     ...
     * constructed.
     *
     * @param[in] aFilename         UTF-8 path/uri to the database file
     * ("filename" sqlite3 parameter)
     * @param[in] aFlags
     * SQLite::OPEN_READONLY/SQLite::OPEN_READWRITE/SQLite::OPEN_CREATE...
     * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before
     * returning SQLITE_BUSY (see setBusyTimeout())
     * @param[in] aVfs              UTF-8 name of custom VFS to use, or empty
     * string for sqlite3 default
     *
     * @throw SQLite::Exception in case of error
     */
    Database (const std::string &aFilename,
      ...
      : Database (aFilename.c_str (), aFlags, aBusyTimeoutMs,
     ...
     * constructed.
     *
     * @param[in] apFilename        Path/uri to the database file ("filename"
     * sqlite3 parameter)
     * @param[in] aFlags
     * SQLite::OPEN_READONLY/SQLite::OPEN_READWRITE/SQLite::OPEN_CREATE...
     * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before
    ...
    Database (const std::filesystem::path &apFilename,
              ...
              const int aBusyTimeoutMs = 0, const std::string &aVfs = "")
      : Database (
          reinterpret_cast<const char *> (apFilename.u8string ().c_str ()),
     ...
     *
     *  This is useful in multithreaded program to handle case where a table is
     * locked for writing by a thread. Any other thread cannot access the table
     * and will receive a SQLITE_BUSY error: setting a timeout will wait and
     * retry up to the time specified before returning this SQLITE_BUSY error.
     *  Reading the value of timeout for current connection can be done with
     * SQL query "PRAGMA busy_timeout;". Default busy timeout is 0ms.
     *
     * @param[in] aBusyTimeoutMs    Amount of milliseconds to wait before
     ...
     *
     *  This is useful for any kind of statements other than the Data Query
     * Language (DQL) "SELECT" :
     ...
     * @see Statement::exec() to handle precompiled statements (for better
     * performances) without results
     * @see Statement::executeStep() to handle "SELECT" queries with results
     *
     * @param[in] apQueries  one or multiple UTF-8 encoded, semicolon-separate
     * SQL statements
     *
     * @return number of rows modified by the *last* INSERT, UPDATE or DELETE
     * statement (beware of multiple statements)
     * @warning undefined for CREATE or DROP table: returns the value of a
     * previous INSERT, UPDATE or DELETE statement.
     *
     * @throw SQLite::Exception in case of error
     */
     ...
     * @see Statement::executeStep() to handle "SELECT" queries with results
     *
     * @param[in] aQueries  one or multiple UTF-8 encoded, semicolon-separate
     * SQL statements
     *
     * @return number of rows modified by the *last* INSERT, UPDATE or DELETE
     * statement (beware of multiple statements)
     ...
     * @throw SQLite::Exception in case of error
     */
    int exec (const std::string &aQueries) { return exec (aQueries.c_str ()); }

    /**
     * @brief Try to execute one or multiple statements, returning the sqlite
     * result code.
     *
     *  This is useful for any kind of statements other than the Data Query
     * Language (DQL) "SELECT" :
    ...
    /**
     * @brief Shortcut to execute a one step query and fetch the first column
     * of the result.
     *
     *  This is a shortcut to execute a simple statement with a single result.
     * This should be used only for non reusable queries (else you should use a
     * Statement with bind()). This should be used only for queries with
     * expected results (else an exception is fired).
     *
     * @warning WARNING: Be very careful with this dangerous method: you have
     * to make a COPY OF THE result, else it will be destroy before the next
     * line (when the underlying temporary Statement and Column objects are
     * destroyed)
     *
    ...
    /**
     * @brief Get the rowid of the most recent successful INSERT into the
     * database from the current connection.
     *
     * Each entry in an SQLite table always has a unique 64-bit signed integer
     * key called the rowid. If the table has a column of type INTEGER PRIMARY
     * KEY, then it is an alias for the rowid.
     *
     * @return Rowid of the most recent successful INSERT into the database, or
     * 0 if there was none.
     */
    ...
    /// statement since connection (not DROP table).
    ...
    const std::string &getFilename () const noexcept { return mFilename; }

    /**
     * @brief Return raw pointer to SQLite Database Connection Handle.
     *
     * This is often needed to mix this wrapper with other libraries or for
     * advance usage not supported by SQLiteCpp.
     */
    sqlite3 *getHandle () const noexcept { return mSQLitePtr.get (); }

    /**
     * @brief Create or redefine a SQL function or aggregate in the sqlite
     * database.
     *
     ...
     *
     * @param[in] apFuncName    Name of the SQL function to be created or
     * redefined
     * @param[in] aNbArg        Number of arguments in the function
     * @param[in] abDeterministic Optimize for deterministic functions (most
     * are). A random number generator is not.
     * @param[in] apApp         Arbitrary pointer of user data, accessible with
     * sqlite3_user_data().
     * @param[in] apFunc        Pointer to a C-function to implement a scalar
     * SQL function (apStep & apFinal nullptr)
     * @param[in] apStep        Pointer to a C-function to implement an
     * aggregate SQL function (apFunc nullptr)
     * @param[in] apFinal       Pointer to a C-function to implement an
     * aggregate SQL function (apFunc nullptr)
     * @param[in] apDestroy     If not nullptr, then it is the destructor for
    ...
    void createFunction (
      ...
      = nullptr, // NOLINT(readability/casting)
      void (*apDestroy) (void *) = nullptr);

    /**
     * @brief Load a module into the current sqlite database instance.
     *
     *  This is the equivalent of the sqlite3_load_extension call, but
     * additionally enables module loading support prior to loading the
     * requested module.
     *
     ...
     * @note UTF-8 text encoding assumed.
     *
     * @param[in] apExtensionName   Name of the shared library containing
     * extension
     * @param[in] apEntryPointName  Name of the entry point (nullptr to let
     * sqlite work it out)
     *
     * @throw SQLite::Exception in case of error
     */
     ...
     *
     *  This is the equivalent of the sqlite3_key call and should thus be
     * called directly after opening the database. Open encrypted database ->
     * call db.key("secret") -> database ready
     *
     ...
     *
     *  This is the equivalent of the sqlite3_rekey call and should thus be
     * called after the database has been opened with a valid key. To decrypt a
     *  database, call this method with an empty string.
     *  Open normal database -> call db.rekey("secret") -> encrypted database,
     * database ready Open encrypted database -> call db.key("secret") -> call
     * db.rekey("newsecret") -> change key, database ready Open encrypted
     * database -> call db.key("secret") -> call db.rekey("") -> decrypted
    ...
    Header getHeaderInfo () const { return getHeaderInfo (mFilename); }
    ...
    /**
     * @brief Check if aRet equal SQLITE_OK, else throw a SQLite::Exception
  ...
  private:
    // TODO: perhaps switch to having Statement sharing a pointer to the
    // Connexion
    std::unique_ptr<sqlite3, Deleter>
      mSQLitePtr;          ///< Pointer to SQLite Database Connection Handle
