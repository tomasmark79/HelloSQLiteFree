 ...
 * @file    Savepoint.h
 * @ingroup SQLiteCpp
 * @brief   A Savepoint is a way to group multiple SQL statements into an
 * atomic secured operation. Similar to a transaction while allowing child
 * savepoints.
 *
...
namespace SQLite {
   ...
   * COMMIT, except that the SAVEPOINT and RELEASE commands are named and may
   * be nested..
   *
   * Resource Acquisition Is Initialization (RAII) means that the Savepoint
   * begins in the constructor and is rolled back in the destructor (unless
   * committed before), so that there is no need to worry about memory
   * management or the validity of the underlying SQLite Connection.
   *
   ...
   * Caveats:
   *
   * 1) Calling COMMIT or committing a parent transaction or RELEASE on a
   * parent savepoint will cause this savepoint to be released.
   *
   * 2) Calling ROLLBACK TO or rolling back a parent savepoint will cause this
   * savepoint to be rolled back.
   *
   * 3) This savepoint is not saved to the database until this and all
   * savepoints or transaction in the savepoint stack have been released or
   * committed.
   *
   * See also: https://sqlite.org/lang_savepoint.html
   *
   * Thread-safety: a Savepoint object shall not be shared by multiple threads,
