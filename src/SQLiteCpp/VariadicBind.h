...
namespace SQLite {
   ...
   *
   * \code{.cpp}
   * SQLite::Statement stm("SELECT * FROM MyTable WHERE colA>? && colB=? &&
   * colC<?"); SQLite::bind(stm,a,b,c);
   * //...is equivalent to
   * stm.bind(1,a);
   * stm.bind(2,b);
   * stm.bind(3,c);
  ...
  /**
   * \brief Convenience function for calling Statement::bind(...) once for each
   * parameter of a tuple, by forwarding them to the variadic template
   *
   * This feature requires a c++14 capable compiler.
   *
   * \code{.cpp}
   * SQLite::Statement stm("SELECT * FROM MyTable WHERE colA>? && colB=? &&
   * colC<?"); SQLite::bind(stm, std::make_tuple(a, b, c));
   * //...is equivalent to
   * stm.bind(1,a);
   * stm.bind(2,b);
   * stm.bind(3,c);
  ...
  /**
   * \brief Convenience function for calling Statement::bind(...) once for each
   * parameter of a tuple, by forwarding them to the variadic template. This
   * function is just needed to convert the tuples to parameter packs
   *
   * This feature requires a c++14 capable compiler.
   *
   * @param query     statement
