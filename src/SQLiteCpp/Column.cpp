...
namespace SQLite
{
  ...
  // Encapsulation of a Column in a row of the result pointed by the prepared
  // Statement.
  ...
  // Return the name of the table column that is the origin of this result
  // column
  ...
  // Return the integer value of the column specified by its index starting at
  // 0
  ...
  // Return the unsigned integer value of the column specified by its index
  // starting at 0
  ...
  // Return the 64bits integer value of the column specified by its index
  // starting at 0
  ...
  const char *
  Column::getText (const char *apDefaultValue /* = "" */) const noexcept
  {
    auto pText = reinterpret_cast<const char *> (
      sqlite3_column_text (mStmtPtr.get (), mIndex));
    return (pText ? pText : apDefaultValue);
  }

  // Return a pointer to the blob value (*not* NULL terminated) of the column
  // specified by its index starting at 0
  ...
  std::string Column::getString () const
  {
    ...
    // - no need for sqlite3_column_text to add a \0 on the end, as we're
    // getting the bytes length directly
    //   however, we need to call sqlite3_column_bytes() to ensure correct
    //   format. It's a noop on a BLOB or a TEXT value with the correct
    //   encoding (UTF-8). Otherwise it'll do a conversion to TEXT (UTF-8).
    (void)sqlite3_column_bytes (mStmtPtr.get (), mIndex);
    auto data = static_cast<const char *> (
      sqlite3_column_blob (mStmtPtr.get (), mIndex));

    // SQLite docs: "The safest policy is to invoke… sqlite3_column_blob()
    // followed by sqlite3_column_bytes()" Note: std::string is ok to pass
    // nullptr as first arg, if length is 0
