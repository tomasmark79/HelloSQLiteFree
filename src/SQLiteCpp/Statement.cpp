...
// check for if SQLite3 version >= 3.14.0
  ...
  #warning                                                                    \
    "SQLite3 version is less than 3.14.0, so expanded SQL is not available"
  #warning                                                                    \
    "To use expanded SQL, please upgrade to SQLite3 version 3.14.0 or later"
  #warning                                                                    \
    "If you want to disable this warning, define SQLITECPP_DISABLE_SQLITE3_EXPANDED_SQL"
...
namespace SQLite
{
  ...
  Statement::Statement (const Database &aDatabase, const char *apQuery)
    : mQuery (apQuery), mpSQLite (aDatabase.getHandle ()),
      mpPreparedStatement (prepareStatement ()) // prepare the SQL query (needs
                                                // Database friendship)
  {
  ...
  // Reset the statement to make it ready for a new execution (see also
  // #clearBindings() below)
  ...
  // Clears away all the bindings of a prepared statement (can be associated
  // with #reset() above).
  ...
  // Bind an 32bits int value to a parameter "?", "?NNN", ":VVV", "@VVV" or
  // "$VVV" in the SQL prepared statement
  ...
  // Bind a 32bits unsigned int value to a parameter "?", "?NNN", ":VVV",
  // "@VVV" or "$VVV" in the SQL prepared statement
  void Statement::bind (const int aIndex, const uint32_t aValue)
  {
    const int ret
      = sqlite3_bind_int64 (getPreparedStatement (), aIndex, aValue);
    check (ret);
  }

  // Bind a 64bits int value to a parameter "?", "?NNN", ":VVV", "@VVV" or
  // "$VVV" in the SQL prepared statement
  ...
  // Bind a double (64bits float) value to a parameter "?", "?NNN", ":VVV",
  // "@VVV" or "$VVV" in the SQL prepared statement
  void Statement::bind (const int aIndex, const double aValue)
  {
    const int ret
      = sqlite3_bind_double (getPreparedStatement (), aIndex, aValue);
    check (ret);
  }

  // Bind a string value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV"
  // in the SQL prepared statement
  void Statement::bind (const int aIndex, const std::string &aValue)
  {
    const int ret = sqlite3_bind_text (
      getPreparedStatement (), aIndex, aValue.c_str (),
      static_cast<int> (aValue.size ()), SQLITE_TRANSIENT);
    check (ret);
  }

  // Bind a text value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in
  // the SQL prepared statement
  void Statement::bind (const int aIndex, const char *apValue)
  {
    const int ret = sqlite3_bind_text (getPreparedStatement (), aIndex,
                                       apValue, -1, SQLITE_TRANSIENT);
    check (ret);
  }

  // Bind a binary blob value to a parameter "?", "?NNN", ":VVV", "@VVV" or
  // "$VVV" in the SQL prepared statement
  void Statement::bind (const int aIndex, const void *apValue, const int aSize)
  {
    const int ret = sqlite3_bind_blob (getPreparedStatement (), aIndex,
                                       apValue, aSize, SQLITE_TRANSIENT);
    check (ret);
  }
  ...
  {
    const int ret
      = sqlite3_bind_text (getPreparedStatement (), aIndex, aValue.c_str (),
  ...
  {
    const int ret = sqlite3_bind_text (getPreparedStatement (), aIndex,
                                       apValue, -1, SQLITE_STATIC);
    check (ret);
  }

  // Bind a binary blob value to a parameter "?", "?NNN", ":VVV", "@VVV" or
  // "$VVV" in the SQL prepared statement
  void Statement::bindNoCopy (const int aIndex, const void *apValue,
                              const int aSize)
  {
    const int ret = sqlite3_bind_blob (getPreparedStatement (), aIndex,
                                       apValue, aSize, SQLITE_STATIC);
    check (ret);
  }

  // Bind a NULL value to a parameter "?", "?NNN", ":VVV", "@VVV" or "$VVV" in
  // the SQL prepared statement
  ...
  // Execute a one-step query with no expected result, and return the number of
  // changes.
  int Statement::exec ()
  {
    const int ret = tryExecuteStep ();
    if (SQLITE_DONE
        != ret) // the statement has finished executing successfully
    {
      if (SQLITE_ROW == ret)
      {
  ...
  // Return a copy of the column data specified by its column name starting at
  // 0 (use the Column copy-constructor)
  ...
  bool Statement::isColumnNull (const int aIndex) const
  {
    ...
    checkIndex (aIndex);
    return (SQLITE_NULL
            == sqlite3_column_type (getPreparedStatement (), aIndex));
  }
  ...
  bool Statement::isColumnNull (const char *apName) const
  {
    ...
    const int index = getColumnIndex (apName);
    return (SQLITE_NULL
            == sqlite3_column_type (getPreparedStatement (), index));
  }

  // Return the named assigned to the specified result column (potentially
  // aliased)
  ...
  const char *Statement::getColumnDeclaredType (const int aIndex) const
  {
    checkIndex (aIndex);
    const char *result
      = sqlite3_column_decltype (getPreparedStatement (), aIndex);
    if (!result)
    {
  ...
  std::string Statement::getExpandedSQL () const
  {
    ...
    throw SQLite::Exception (
      "this version of SQLiteCpp does not support expanded SQL");
#else
  ...
  Statement::TStatementPtr Statement::prepareStatement ()
  {
    ...
    const int ret = sqlite3_prepare_v2 (mpSQLite, mQuery.c_str (),
