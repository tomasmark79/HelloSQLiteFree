...
namespace SQLite {
   ...
   *
   * A Backup object is used to backup a source database file to a destination
   * database file in a safe and online way.
   *
  ...
  public:
     ...
     *
     * Initialize a SQLite Backup object for the source database and
     * destination database. The database name is "main" for the main database,
     * "temp" for the temporary database, or the name specified after the AS
     * keyword in an ATTACH statement for an attached database.
     *
     * Exception is thrown in case of error, then the Backup object is NOT
     * constructed.
     *
     ...
     * @brief Initialize a SQLite Backup object.
     *
     * Initialize a SQLite Backup object for source database and destination
     * database. The database name is "main" for the main database, "temp" for
     ...
     *
     * Initialize a SQLite Backup object for source database and destination
     * database. Backup the main databases between the source and the
     * destination.
     *
     * Exception is thrown in case of error, then the Backup object is NOT
     * constructed.
     ...
     * be copied
     *
     * Exception is thrown when SQLITE_IOERR_XXX, SQLITE_NOMEM, or
     * SQLITE_READONLY is returned in sqlite3_backup_step(). These errors are
     * considered fatal, so there is no point in retrying the call to
     * executeStep().
     *
     * @param[in] aNumPage    The number of source pages to be copied, with a
     * negative value meaning all remaining source pages
     *
    ...
    /// Return the number of source pages still to be backed up as of the most
    /// recent call to executeStep().
